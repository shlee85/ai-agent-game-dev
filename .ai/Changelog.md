...
## [2026-05-15] 그리드 9×16 → 7×12 축소 + edge-to-edge 맵 (`c9e72db`)

### 상세 변경 사항 요약

1. **그리드 규격 변경** — 9행×16열 → 7행×12열
   - tileSize: 45px → 58px (landscape 기준)
   - 캐릭터/타워 가시성: 27px → 35px+ (Kingdom Rush 수준)

2. **20개 웨이브 경로 전면 재설계** — 새 그리드에 맞게 모든 경로 신규 설계
   - Wave 3/18/20 듀얼 패스 유지
   - Wave 1/2 장애물 타일 유지

3. **GridMap edge-to-edge** — 테두리·패딩·배경 제거, 맵이 화면 전체 차지
