...
export default function App() {
  ...
  useEffect(() => {
    const loadMeta = async () => {
      const minTimer = new Promise<void>((resolve) => setTimeout(resolve, LOADING_MIN_MS));
      const dataLoad = async () => {
        try {
          ...
          if (savedDiamond != null) setDiamond(Number(savedDiamond));
          if (savedInventory) setItemInventory({ ...DEFAULT_ITEM_INVENTORY, ...JSON.parse(savedInventory) });
        } catch (e) {
          console.error("Failed to load meta shop state", e);
        }
      };
      await Promise.all([minTimer, dataLoad()]);
      setIsLoading(false);
    };
    loadMeta();
  }, []);
